...
int main() {
    ...
    sort(all(strv), [](string a, string b) { return ln(a) > ln(b); });
    // EVARS(strv);
    
    string res;
    if (strv[0].substr(1, n - 1) == strv[1].substr(0, n - 1)) {
        res = strv[0] + strv[1][n - 1];
    } else
        res = strv[1] + strv[0][n - 1];
    EVARS(res);
    vi a(k);
    here:
    for (c = 0; c < k; c += 2) {
        if (strv[c] == res.substr(0, n - c/2)) {
            a[c] = 1;
            // a[c+1]=2;
        } else if(strv[c+1] == res.substr(0, n - c/2)){
            // a[c]=2;
            a[c + 1] = 1;
        }
        else {
            EVARS(res.substr(0, n - c / 2));
            // cout<<sub;
            res = strv[1] + strv[0][n - 1];
            for(c = 0; c < n; c++){
                a[c]=0;
    ...
    EVARS(res);
    for (c = 0; c < k; c++) {
        auto it = find(all(strv), temp[c]);
        EVARS(it - strv.begin());
        if (a[it - strv.begin()])
            cout << 'P';
        else
            cout << 'S';
        strv[it - strv.begin()] = '1';
