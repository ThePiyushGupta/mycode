...
int main()
{
    ...
    for (c = 0; c < n; c++)
    {
        if (c & 1)
        {
            if (c == 1)
            {
                if (se[sz(se) - 1] - 2 * se[0] == so[sz(so) - 1] - so[0])
                    res++;
            }
            else
            {
                if (se[sz(se) - 1] - 2 * se[c / 2] == so[sz(so) - 1] - so[c / 2 - 1] - so[c / 2])
                    res++;
                EVARS(se[sz(se) - 1] - 2 * se[c / 2], so[sz(so) - 1] - so[c / 2 - 1] - so[c / 2], c);
            }
        }
        else
        {
            if (c == 0)
            {
                if (se[sz(se) - 1] - se[0] == so[sz(so) - 1])
                    res++;
            }
            else
            {
                if (se[sz(se) - 1] + -se[c / 2] - se[c / 2 - 1] == so[sz(so) - 1] - 2 * so[c / 2 - 1])
                    res++;
                EVARS(se[sz(se) - 1] + -se[c / 2] - se[c / 2 - 1], so[sz(so) - 1] - 2 * so[c / 2 - 1], c);
            }
        }
    }
    cout << res;
