...
int main()
{
    ...
    vector<ll> a(k), b(k), d(n + 1, 0), ctz(n, 0);
    for (c = 0; c < k; c++)
    {
        cin >> a[c] >> b[c];
    }
    ...
    vector<pair<int, pi>> res(k);
    EVARS(ctz);
    for (c = 0; c < k; c++)
    {
        res[c].F = ctz[b[c]] - ctz[a[c] - 1];
        EVARS(res[c].F);
        res[c].S.F = b[c] - a[c] + 1;
        res[c].S.S = c;
    }
    sort(all(res), cmp);
    sort(all(res), by(F));
    a.erase(a.begin() + res[0].S.S);
    b.erase(b.begin() + res[0].S.S);
    k--;
    ans += res[0].F;
    EVARS(res[0].S.S, ans, res[0].F);
    res.resize(k);
