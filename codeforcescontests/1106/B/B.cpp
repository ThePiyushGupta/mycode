...
// #define by() [](const auto &a, const auto &b) {  }
...
int c;//predeclared control variables for loops
//************************************************************************************************************

bool by(pair<pi,long> a,pair<pi,long> b){
    if(a.F.S==b.F.S)
        return a.S>b.S;
...
int main()
{
    ...
    for(c = 0; c < m; c++)
    {
        ...
        if(kind<sz(a)&&a[kind].F.F>=count){
            cout<<count*a[kind].F.S<<endl;
            a[kind].F.F-=count;
        }
