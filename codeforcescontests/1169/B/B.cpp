...
int support(vector<pi> a, int val)
{
    int n = a.size();
    vector<int> temp(k + 10);
    EVARS(n, k);
    for (int c = 0; c < n; ++c)
    {
        if (a[c].first == val || a[c].second == val)
            temp[val]++;
        else
            temp[a[c].first]++, temp[a[c].second]++;
    }
    for (int c = 0; c < temp.size(); c++)
    {
        if (c == val)
            continue;
        if (temp[c] + temp[val] == n)
        {
            EVARS(c, val);
            return 1;
        }
...
int main()
{
    ...
    // EVARS(support(a, a[0].first), support(a, a[0].second));

    if (support(a, a[0].first) || support(a, a[0].second))
        cout << "YES";
    else
        cout << "NO";
}